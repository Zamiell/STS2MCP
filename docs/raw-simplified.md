...
| `get_replays` | _(none)_ | List all replay floors currently discoverable by the RunReplays mod. Returns entries and grouped seeds; each entry includes a `target` value that can be passed to `start_replay`. |
| `start_replay` | `target`?: string, `seed`?: string, `floor`?: int, `start_floor`?: int | Start a replay through the RunReplays mod. Requires RunReplays to be installed and enabled. Use `target: "SEED"` or `target: "SEED:floor_N"`, pass `seed` with optional numeric `floor`, or pass `seed`, `floor`, and `start_floor` to load a saved floor and replay to a later floor. |
| `debug_start_encounter` | `encounter`: string | Start a specific encounter immediately in the active singleplayer run. Accepts model class names such as `ChompersNormal` or normalized names such as `chompers-normal`. Intended for validation/debug workflows; do not call while already in combat. |
