...
| `start_replay` | `target`?: string, `seed`?: string, `floor`?: int | Start a replay through the RunReplays mod. Requires RunReplays to be installed and enabled. Use `target: "SEED"` or `target: "SEED:floor_N"`, or pass `seed` with optional numeric `floor`. |
